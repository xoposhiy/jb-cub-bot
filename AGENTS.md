...
- `uv sync` — set up env
- `uv run pytest` — tests
- `uv run python -m sdt_bot` — run the bot (needs `.env`; see `.env.example`)
- Alembic (`uv run alembic ...`) **requires a populated `.env`** — `alembic/env.py` loads `get_settings()`.

## Conventions that aren't obvious
- **Add a feature** = a package in `src/sdt_bot/features/<name>/` exporting `router` (aiogram `Router`) + `manifest`. The loader auto-discovers it — no central edits.
